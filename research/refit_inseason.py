...
def main(inputs, output):
 ...
 historical_count=len(h)
 # Apply audited corrections since the previous fit (e.g. USC–SJSU 2,000K).
 actuals=pd.read_csv(old/'viewership_cleaned.csv').set_index('source_index')['Persons 2+']
 corrections=[]
 for idx,r in h.iterrows():
  value=float(actuals.loc[int(r.source_index)])
  if value!=float(r['Persons 2+']):corrections.append(dict(source_index=int(r.source_index),before=float(r['Persons 2+']),after=value))
  h.loc[idx,'Persons 2+']=value
 ...
 assert len(h)==historical_count+len(rows) and h.source_index.is_unique
 ...
 daily=copy.deepcopy(json.loads((old/'audience_interest_data.json').read_text())['daily'])
 ...
 through=pd.to_datetime(h.ParsedDate).max().date().isoformat()
 metadata=dict(training_max_year=int(h.Year.max()),prediction_year=2026,training_through_date=through,training_rows=len(h),
 ...
 nonlinear['description']=f'Same validated nonlinear blend, refitted through {through}; historical OOS calibration retained.'
 ...
 post['description']=f'Full pregame architecture plus final absolute score differential; refitted through {through}.'
 ...
 interest['feature_training_matches']=int(z[FEATURE].sum())
 ...
 if (old/'marquee_calibration.json').exists():
  config=json.loads((old/'marquee_calibration.json').read_text())
  config['base_sha256']={n:digest(n) for n in config['base_sha256']}
  config['refit_note']='Frozen 2024–25 held-out calibration retained during coefficient refit; no fit to the new actuals.'
  (output/'marquee_calibration.json').write_text(json.dumps(config,indent=2)+'\n')
 ...
 report=dict(**metadata,added_games=len(rows),added_by_week={str(w):sum(r['week']==w for r in rows) for w in sorted({r['week'] for r in rows})},historical_target_corrections=corrections,actual_viewers_000s=sum(r['Persons 2+'] for r in payload['clean']['rows']),
             historical_rows_preserved=historical_count,attention_training_coverage=len(attention),primary_features=len(x.columns),
