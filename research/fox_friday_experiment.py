"""FOX regular Friday-night interaction. Strict rolling-season fits; no production writes."""
import json,sys
...
O=B/'research/fox_friday_results';O.mkdir(exist_ok=True)
MAJOR=['ABC','CBS','FOX','NBC','ESPN']
SPECS={'current':None,'fox_friday_night':None}
...
def fit_variant(a,b,tr,te,context,y,variant,base):
 ...
 aa=a.copy();bb=b.copy();aa[FEATURE]=interaction(tr,a);bb[FEATURE]=interaction(te,b)
 ...
 for branch,cs in result[1].items():
  for i,c in enumerate(cs):
   if FEATURE in c['columns']:
    ...
    audit.append(dict(branch=branch,component=i,coefficient=coef,multiplier=float(np.exp(coef)),train_matches=int(aa[FEATURE].sum()),test_matches=int(bb[FEATURE].sum())))
 return result,audit
...
def main():
 ...
 classification[FEATURE]=interaction(h,h);classification.to_csv(O/'classification.csv',index=False)
 ...
 ref=pd.read_csv(R/'audience_interest_weekly/predictions.csv');ref=ref[ref.variant.eq('interest_level')].set_index('source_index')
 ...
 summary=[];annual=[];ci=[]
 scopes={'all':p.Year.gt(0),'major':p.Station.isin(MAJOR),'FOX':p.Station.eq('FOX'),'target':p.source_index.isin(classification.loc[classification[FEATURE].eq(1),'source_index']),'target_2025':p.Year.eq(2025)&p.source_index.isin(classification.loc[classification[FEATURE].eq(1),'source_index']),'major_2024_2025':p.Station.isin(MAJOR)&p.Year.ge(2024),'week1_major':p.Station.isin(MAJOR)&p.period.eq('Week1')}
 ...
 print('Kansas-Missouri:',flush=True);print(preview[(preview['Team 1'].eq('Missouri')|preview['Team 2'].eq('Missouri'))][['variant','pred','actual']].to_string(index=False),flush=True)
