"""Rolling-season slate hierarchy experiment; no production writes."""
import sys
...
from audience_interest import FEATURES
from team_half_life_experiment import metrics
O=B/'research/slate_position_results';O.mkdir(exist_ok=True)
MAJOR=['ABC','CBS','FOX','NBC','ESPN']
SPECS={'current':(), 'own_strength':('SlateOwnLogAudience',),
 'leader':('SlateOwnLogAudience','SlateLeader','CompetitionXLeader'),
 'share':('SlateOwnLogAudience','SlateAudienceShare','CompetitionXShare'),
 'stronger':('SlateOwnLogAudience','StrongerCompetition'),
 'leader_only':('SlateLeader','CompetitionXLeader'),
 'share_only':('SlateAudienceShare','CompetitionXShare'),
 'stronger_only':('StrongerCompetition',)}
from slate_position import slate_features
from viewership_competition import cross_fitted_intrinsic_predictions,fit_intrinsic_audience_model,predict_intrinsic_viewers
from viewership_feature_contract import BASE_NUMERIC_FEATURES
...
def fit_variant(a,b,tr,te,context,y,variant,base):
 ...
 for branch,cs in result[1].items():
  for i,c in enumerate(cs):
   for feature in (*SPECS[variant], 'Competing_Games_Score'):
    if feature in c['columns']:
     coef=float(c['coefficients'][c['columns'].index(feature)])
     audit.append(dict(feature=feature,branch=branch,component=i,coefficient=coef,multiplier=float(np.exp(coef))))
 return result,audit
...
def main():
 ...
 for year in [2019,2021,2022,2023,2024,2025]:
  ...
  train_intrinsic=cross_fitted_intrinsic_predictions(tr,BASE_NUMERIC_FEATURES)
  fit=fit_intrinsic_audience_model(tr,BASE_NUMERIC_FEATURES);test_intrinsic=predict_intrinsic_viewers(fit,te)
  for frame,x,intrinsic,split in [(tr,a,train_intrinsic,'train'),(te,b,test_intrinsic,'test')]:
   sf=slate_features(frame,intrinsic)
   np.testing.assert_allclose(sf.CompetitionTotal,x.Competing_Games_Score,atol=1e-8)
   frame=frame.copy()
   for c in sf:frame[c]=sf[c]
   audit=sf.copy();audit['holdout_year']=year;audit['split']=split;audit['source_index']=frame.source_index;hierarchy_audit.append(audit)
   if split=='train':tr=frame
   else:te=frame
 ...
 pd.concat(hierarchy_audit).to_csv(O/'feature_audit.csv',index=False)
 classification=pd.concat([g for g in hierarchy_audit if g.split.iloc[0]=='test']).set_index('source_index')
 ...
 summary=[];annual=[];ci=[]
 scopes={'all':p.Year.gt(0),'major':p.Station.isin(MAJOR),'FOX':p.Station.eq('FOX'),
 'slate_leaders':p.source_index.isin(classification.index[classification.SlateLeader.eq(1)]),
 'lower_draws':p.source_index.isin(classification.index[classification.SlateRank.ge(3)]),
 'major_2024_2025':p.Station.isin(MAJOR)&p.Year.ge(2024),'week1_major':p.Station.isin(MAJOR)&p.period.eq('Week1')}
 ...
 pd.DataFrame(coeffs).to_csv(O/'coefficients.csv',index=False)
