...
class InseasonDataTests(unittest.TestCase):
    def test_actuals_are_unique_and_available_for_both_teams(self):
        ...
        self.assertEqual(len(data),2453);self.assertEqual(len(new),64)
        ...
        self.assertAlmostEqual(new['Persons 2+'].sum(),139126)
    ...
    def test_paired_bundle_includes_all_new_observations(self):
        ...
        self.assertEqual(model.nobs,2453)
        self.assertEqual(model.audience_interest['training_rows'],2453)
        self.assertEqual(model.audience_interest['prediction_year'],2026)
        self.assertEqual(post['training_through_date'],'2026-09-12')
